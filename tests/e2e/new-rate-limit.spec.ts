 ...
 * `workers: 1`) — the per-client `createSession` budget (15/10 min, 40/h) was sized with
...
test("repeated POST /new from the same client trips the rate limiter with Retry-After", async ({ page }) => {
  ...
  // Up to 20 attempts: enough to trip the 15/10min per-client limit even starting from a
