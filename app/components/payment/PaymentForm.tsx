...
export function PaymentForm({
  ...
  return (
    <Form method="post" className="flex flex-col gap-6">
      ...
      <Field htmlFor="payerPublicId" label={t("payment.fromLabel")} error={fieldError("payerPublicId")}>
        {(ids) => (
      ...
      <Field
        ...
        error={fieldError("recipientPublicId") ?? (samePerson ? t("payment.samePersonHint") : undefined)}
      >
        {(ids) => (
          <Select {...ids} name="recipientPublicId" value={recipientPublicId} onChange={(e) => setRecipientPublicId(e.target.value)}>
            {participants.map((p) => (
      ...
      <div className="flex items-end gap-3">
        <Field htmlFor="amountText" label={t("payment.amountLabel")} error={fieldError("amountText")} className="flex-1">
          {(ids) => (
            <MoneyInput
              {...ids}
              name="amountText"
      ...
      <Field htmlFor="paymentDate" label={t("payment.dateLabel")} error={fieldError("paymentDate")}>
        {(ids) => (
          <Input {...ids} type="date" name="paymentDate" required value={paymentDate} onChange={(e) => setPaymentDate(e.target.value)} />
        )}
      </Field>
      ...
      <Button type="submit" size="lg" loading={submitting} disabled={submitting || samePerson} fullWidth>
        {submitLabel}
      </Button>
    </Form>
  );
