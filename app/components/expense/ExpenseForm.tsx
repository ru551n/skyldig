...
export interface ParticipantOption {
   ...
   * split preview so it matches what the server will store (the rounding
   * remainder in `splitEqually` is assigned by position).
   */
  position: number;
}
  ...
  const preview = useMemo(() => {
    try {
      ...
      const parts = participants
        ...
        .map((p) => ({ id: p.publicId, position: p.position }));
